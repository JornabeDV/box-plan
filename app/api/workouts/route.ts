...
export async function GET(request: NextRequest) {
  try {
    ...
    // Verificar propiedad: solo el propio usuario o su coach pueden ver los workouts
    if (targetUserId !== sessionUserId) {
      ...
      if (!coachCheck.isAuthorized || !(await isCoachOfStudent(coachCheck.profile!.id, targetUserId))) {
        return NextResponse.json({ error: 'No autorizado para ver estos workouts' }, { status: 403 })
      }
    }
