...
interface EditWODModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  wod: WOD | null
  onSubmit: (wodId: string, data: any) => Promise<{ error?: string }>
}

export function EditWODModal({ 
  open, 
  onOpenChange, 
  wod,
  onSubmit 
  ...
  // Cargar datos del WOD cuando se abre el modal
  useEffect(() => {
    if (wod && open) {
      setFormData({
        name: wod.name || '',
        description: wod.description || '',
        type: wod.type || 'metcon',
        difficulty: wod.difficulty || 'beginner',
        duration_minutes: wod.duration_minutes?.toString() || '',
        exercises: wod.exercises || [],
        instructions: wod.instructions || '',
        scaling: wod.scaling || '',
        tips: wod.tips || [],
        is_public: wod.is_public || false,
        is_template: wod.is_template || false,
        date: wod.date || ''
      })
      setError('')
      setActiveTab('basic')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await onSubmit(wod.id, {
        ...formData,
      ...
      setError('Error al actualizar el WOD')
    } finally {
      setLoading(false)
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Modifica los detalles del Workout of the Day seleccionado.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading || !formData.name || !formData.description}>
              {loading ? 'Actualizando...' : 'Actualizar WOD'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
