...
export function UserEditModal({
  ...
  useEffect(() => {
    if (user) {
      setFormData({
        ...
        preferred_discipline_id:
          user.preferences?.preferred_discipline_id || "none",
  ...
  const getDisciplineLevels = (disciplineId: string | null) => {
    ...
    return disciplineLevels.filter(
      (level) => level.discipline_id === disciplineId,
    );
  };
    ...
    try {
      ...
      if (Object.keys(updateBody).length === 0) {
        ...
        const result = await updateUserPreferences(user.id, {
          preferred_discipline_id:
            formData.preferred_discipline_id === "none"
              ? null
              : formData.preferred_discipline_id || null,
          preferred_level_id:
            formData.preferred_level_id === "none"
              ? null
              : formData.preferred_level_id || null,
        });
            ...
            <div className="space-y-2 w-full">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFormData((prev) => ({
                    ...prev,
                    preferred_level_id: value === "none" ? "" : value,
                  }))
                }
                disabled={
                  !formData.preferred_discipline_id ||
                  formData.preferred_discipline_id === "none"
                }
              >
                <SelectTrigger className="w-full">
              ...
              {(!formData.preferred_discipline_id ||
                ...
                <p className="text-sm text-muted-foreground">
                  Selecciona una disciplina para habilitar la selección de
                  nivel.
                </p>
              )}
            </div>
                ...
                {user.preferences.discipline ? (
                  <Badge variant="outline" className="flex items-center gap-1">
                    <div
                      ...
                      style={{
                        backgroundColor: user.preferences.discipline.color,
                      }}
                    />
