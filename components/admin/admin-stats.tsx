...
import { FileText, Users, DollarSign, Calendar, UserPlus, AlertCircle, TrendingUp, UserX, Target, Star } from 'lucide-react'
...
export function AdminStats({ users, planifications, onTabChange }: AdminStatsProps) {
  // Calcular métricas
  ...
  const disciplineUsage = new Map<string, { count: number; discipline: string; level: string }>()
  ...
  const topDiscipline = Array.from(disciplineUsage.values())
  ...
  const mostPopularPlan = Array.from(planCounts.entries())
    .sort((a, b) => b[1] - a[1])[0]
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Disciplina Más Utilizada</CardTitle>
          <Target className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          {topDiscipline ? (
            <>
              <div className="text-2xl font-bold">{topDiscipline.discipline}</div>
              <p className="text-xs text-muted-foreground">
