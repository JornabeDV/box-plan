...
import { CheckCircle2, XCircle, Link2, Loader2, ExternalLink } from 'lucide-react'
import {
	Dialog,
	DialogContent,
	DialogDescription,
...
export function MercadoPagoConnect({ coachId }: MercadoPagoConnectProps) {
	...
	useEffect(() => {
		...
		if (params.get('mercadopago_connected') === 'true') {
			...
			const newUrl = tab ? `${window.location.pathname}?tab=${tab}` : window.location.pathname
	...
	const handleManualSave = async () => {
		...
		try {
			...
			toast({
				...
				description: 'Tu Account ID de MercadoPago se ha guardado correctamente',
			})
			...
			toast({
				title: 'Error',
				description: error instanceof Error ? error.message : 'Error al guardar Account ID',
				variant: 'destructive',
			})
		} finally {
			setIsSaving(false)
					...
					{connected ? (
						<div className="space-y-4">
							...
							<p className="text-sm text-muted-foreground">
								Los pagos de tus estudiantes se distribuirán automáticamente entre tu cuenta y la
								plataforma.
							</p>
							<Button
								variant="outline"
						...
						<div className="space-y-4">
							...
							<p className="text-sm text-muted-foreground">
								Para recibir pagos automáticamente, necesitas conectar tu cuenta de MercadoPago.
							...
							<div className="p-3 bg-muted rounded-lg text-sm text-muted-foreground">
								<p className="font-medium mb-1">¿Dónde encuentro mi Account ID?</p>
			...
			<Dialog open={isManualDialogOpen} onOpenChange={setIsManualDialogOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Ingresa tu Account ID de MercadoPago. Lo puedes encontrar en tu panel de
							MercadoPago.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 py-4">
						<div className="space-y-2">
							...
							<p className="text-xs text-muted-foreground">
								Ingresa solo números. Puedes encontrarlo en tu cuenta de MercadoPago.
							</p>
						</div>
					</div>
					<DialogFooter>
