...
export function CoachPlanCard({ 
	...
	// Planes que tienen período de prueba (Starter y Growth)
	const hasTrialPeriod = plan.name.toLowerCase() === 'starter' || plan.name.toLowerCase() === 'growth'
	...
	const featureList = [
		...
		`Comisión del ${plan.commissionRate}%`,
		features.dashboard && 'Dashboard personalizado',
		...
		>
			...
			<CardHeader className="text-center pb-4">
				...
				<CardDescription className="text-base">
					Para {plan.minStudents === 1 ? '1 estudiante' : `${plan.minStudents}-${plan.maxStudents === 999999 ? 'ilimitados' : plan.maxStudents} estudiantes`}
				</CardDescription>
				
				<div className="mt-6 mb-4">
					<div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg ${colors.bg} border-2 ${colors.border}`}>
						<span className={`text-3xl font-bold ${colors.text}`}>{plan.maxStudents === 999999 ? '∞' : plan.maxStudents}</span>
						<div className="text-left">
							<span className={`block text-sm font-semibold ${colors.text}`}>
								Estudiantes
							</span>
							<span className={`block text-xs ${colors.text} opacity-70`}>máximo</span>
						</div>
					</div>
				</div>
				...
				<div className="mt-4">
					<div className="flex items-baseline justify-center">
						<span className="text-4xl font-bold">{formatPrice(plan.basePrice)}</span>
